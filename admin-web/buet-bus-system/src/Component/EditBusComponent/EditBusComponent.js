...
const EditBusComponent = () => {
    ...
    useEffect(()=>{
        ...
        .then(data => {
            console.log(data);
            setBus(data);
            busName.current.value = data.busName;
            busNo.current.value = data.busNo;
            capacity.current.value = data.capacity;
            regNo.current.value = data.regNo;
            busType.current.value = data.busType;
            dateOfActivation.current.value = data.dateOfActivation;
        })
    ...
    const handleSubmit = (e) =>{
        ...
        const sure = window.confirm("Press OK to update the bus");
        if(!sure){
            return ;
        }
        const url = 'http://localhost:5000/bus/' + id;
  ...
  return (
    <>
        ...
        <p className='text-3xl font-bold text-red-800 my-4 pb-10 text-center'>Edit Bus</p>
        ...
        <form>
            <div className='w-5/6 mx-auto'>
                ...
                <div class="relative z-0 w-full mb-6 group">
                    ...
                    {routes?.map((ele)=>{
                        if(ele._id === bus?.route?._id){
                            return(
                                <option selected value={ele._id}>{ele.routeName}</option>
                                );
                        }
                        else{
                            return(
                ...
                <div class="relative z-0 w-full mb-6 group">
                    ...
                    {staffs?.map((ele)=>{
                        ...
                        if(ele._id === bus?.driver?._id){
                            return(
                                <option selected value={ele._id}>{ele.name} ({ele.staffId})</option>
                                );
                        }
                        else{
                            return(
